...
def compare_countries(request, currency, country1, country2):
    ...
    try:
        ...
        _, created = CountryQuery.objects.get_or_create(country1=sorted_countries[0], country2=sorted_countries[1])
        if not created:
            CountryQuery.objects.filter(country1=sorted_countries[0], country2=sorted_countries[1]).update(num_queries=models.F('num_queries') + 1)
...
def get_api_payments(request):
    ...
    for payment in payments:
        ...
        p['date'] = payment.date.strftime('%Y-%m-%d')

        response.append(p)

    return JsonResponse({"api_payments": response[::-1]})
